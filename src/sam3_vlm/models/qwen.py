...
class RealQwenPlanner:
    ...
    SYSTEM_PROMPT = (
        ...
        "Use simple everyday object names and basic descriptors. Vocabulary is open. "
        "Preserve the user's target object category in every sam3_prompt and missing_appearance_modes entry. "
        "Vary visible appearance only; do not substitute a related object, an object part, or a different "
        "developmental stage. Simple synonyms are allowed only when they refer to the same target objects. "
        "likely_confounders describes other objects that may be mistaken for the target; never promote "
        "those labels into target actions or missing target appearance modes. "
        "Avoid adverbs, stacked nouns, technical jargon, invented compounds, and instructions in these "
        "short phrases. Put all reasoning in rationale. "
    ...
    def plan_scene(self, evidence_pack: QwenEvidencePack, budget: BudgetState, config: V4Config) -> str:
        ...
        text += (
            ...
            "or a missing target appearance mode.\n"
